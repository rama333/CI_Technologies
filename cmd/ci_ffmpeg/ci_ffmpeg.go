...
func (e *Encoder) m4vToTs(input string, output string) (string, string, error) {
	arg := []string{
		e.config.FFMPEG,
		//"FFMPEG:", "/usr/local/bin/ffmpeg", // log level
		//"-rtsp_transport", "tcp",
		//"ffmpeg ",
		"-y",
		"-i", input,
	...
	if err != nil {
		...
		return strings.Join(arg, " "), "stdoutpipe", nil
	}
...
func (e *Encoder) MergeVideo(input1 string, input2 string, output string) (string, string, error) {

	e.m4vToTs(input1, output+"input2.ts")
	e.m4vToTs(input2, output+"_output1.ts")
	input_1 := output + "_output1.ts"
	input_2 := output + "input2.ts"
